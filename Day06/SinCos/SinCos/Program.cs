...
class SinCos
{
    ...
    static void UseSineCosine(double degrees)
    {
        ...
        degrees, sinAngle * sinAngle + cosAngle * cosAngle);// Evaluate sin(2 * X) == 2 * sin(X) * cos(X).
        Console.WriteLine(
        " Math.Sin({0} deg) == {1:E16}",
...
generates the following output.Convert selected values for X to radians
and evaluate these trigonometric identities:
sin^2(X) + cos^2(X) == 1
sin(2 * X) == 2 * sin(X) * cos(X)
...
(Math.Sin(45 deg))^2 + (Math.Cos(45 deg))^2 == 1.0000000000000000E+000Math.Sin(90 deg) == 1.0000000000000000E+000
